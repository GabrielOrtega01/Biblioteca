...
public class UsuarioController {
    ...
    @PostMapping("/solicitarLibro/{id}")
    public String solicitarLibro(@PathVariable Integer id, @RequestParam("libroId") Integer libroId) {
        Usuario usuario = usuarioRepositorio.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("ID de usuario no válido:" + id));
    ...
    @GetMapping("/libros")
    public String listarLibrosUsuarios(Model model) {
        List<Usuario> usuarios = usuarioRepositorio.findAll();
        model.addAttribute("usuarios", usuarios);
        return "usuario/listarLibros";
    }
